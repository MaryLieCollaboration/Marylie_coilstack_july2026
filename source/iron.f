...
c  do iron coils.
CTM  Bessel package ImKmpak included in this file for MaryLie use (4/99)
...
c
c
       subroutine filonin(xmin,xmax,y,nsteps,fname,
     &sinint,cosint)
c  Sign error in endpoints fixed 3/05
...
cc  find Filon weights
	theta=h*y
	call weight(theta,alf,bet,gam)
cc  Zero intermediate arrays.
	do 111 ievod=1,3
	dsinint(ievod)=zero
  111 dcosint(ievod)=zero
c  now perform Filon integration
...
c  Now do end points-first, upper end (xk=xkmx)
	x=xmax
c wrong, error	wt=one 3/05
        wt=-one
	do iend=1,2
	xy=y*x
	cosxy=dcos(xy)
...
ccccccc	f=wt*fname(x)
	...
	x=xmin
c error 3/05    5 wt=-one 
        wt=one
        enddo
c  Now sum up contributions from ievod=1,2,3 with Filon weights.
	sinint=h*(alf*dsinint(3)+bet*dsinint(2)+
     &gam*dsinint(1))
	cosint=h*(alf*dcosint(3)+bet*dcosint(2)+
     &gam*dcosint(1))
	return
	end
c
...
c  Larger x- use downwards recursion
   44 n2=n+m-1
ctm July 2011 PAUSE statement remove for gfortran
ctm   IF (N.LT.0) PAUSE 'bad argument N < 0 in BESSIn'
      IF (N.LT.0) THEN
      WRITE(6,*)'bad argument N < 0 in BESSIn'
      STOP
      ENDIF
ctm end of patch

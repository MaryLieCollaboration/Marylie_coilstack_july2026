...
c set status flag to error value
        imbad=1
        return
      endif
      vv(5,5) = 1.d0
...
c  precision Eispack routines:
      ...
      call dhqr2(mdim,nn,ilo,ihi,aa,reval,aieval,vv,info)
      if ( info .ne. 0 ) then
        write(6,*) '   Something wrong in eig6'
c set status flag to error value
        imbad=3
        return
      endif
...
c perform calculation
   ...
   20 continue
c
c put inverse of "diagonalizing" matrix in buf2m
      call matmat(buf1m,buf2m)
      call inv(buf3a,buf2m)
c
c compute results for buffers 3 through 5
c need to compute buf2m*ji*(buftm transpose) for j=X,Y,Tau.
c
      call matmat(buf2m,t1m)
      call mtran(t1m)
c
      call mclear(t2m)
      t2m(1,1)=1.d0
      t2m(2,2)=1.d0
      call mmult(buf2m,t2m,buf3m)
...
c Modified by Alex Dragt on 22 July 1988 so that mmult(a,a,a)
c makes sense.

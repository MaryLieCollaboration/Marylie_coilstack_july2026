...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
      subroutine isort( iter, dist, nseq, nq)
c
c  subroutine isort sorts the first nq entries of the nseq to the
...
c
   ...
   10 continue
         call isort( iter, dist, nseq, nvec)
c
...
c     write(6,*), ' nls:',nq,'=nq',iter,'=iter',nvec,'=nvec'
      if(iter.lt.0) return
...
c
ctm2014  fv(n) wrong  call gradck( nx, g, grel, sigma, fv, gtol, info)
...
c
c     write(6,*), ' qform:',maxv,'=maxv',nx,'=nx',nvec,'=nvec',nq,'=nq'
...
c
      ...
      stpfrc = 0.1
      if(tol(4).gt.0.0) stpfrc = tol(4)
c     write(6,*), ' qadmin:',nq,'=nq',iter,'=iter',nvec,'=nvec'
      if(iter.lt.0) return
      if (iter .gt. 1) go to 60
c
c      initialize varibles on first iteration
...
c
      call select(maxv,nx,nvec,iter,xcen,xu,fu)
c
c        normalize the working points - xu
c
      if (iscale .eq. 1) then
...
c     	 seed = ran( idint( seed*1.0d8 + 1.0d0) )
        iseed = idint( seed*1.0d8 + 1.0d0)
          call myrand(iseed,ans)
          seed = ans
         write(6,*), seed,' = seed in OPTI RANSTEP'
         xnew(i) = seed - 0.5d0
   10 continue
      cnorm = enorm(nv, xnew)
      xnorm = enorm(nv, xcen)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
      subroutine select(nxmax,nx,nvec,iter,xcen,xu,fu)
c
c  subroutine select selects the xu from among all xv points by
c  choosing the best point, xcen, and the nvec - 1 nearest points
c  to xcen.
c--------------------------------------------------------------------
      include 'impli.inc'
